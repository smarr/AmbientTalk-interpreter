...
import edu.vub.util.regexp.RE;
import edu.vub.util.regexp.REException;
import edu.vub.util.regexp.RESyntax;
...
public class Pattern implements Serializable
{
  ...
  public String[] split (StringBuffer input, int limit)
  {
    ...
    if (limit != 0 || (limit == 0 && !emptyLast))
      {
    String t = input.substring(start, input.length()).toString();
    if ("".equals(t) && limit == 0)
      ; // Don't add.
    else
      list.add(t);
